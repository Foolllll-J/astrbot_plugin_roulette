...
class StatsManager:
    ...
    def record_game_result(self, loser_id: str, winner_ids: List[str], is_pvp: bool = False):
        """
        记录游戏结果
        ...
        with self._lock:
            # 记录失败者
            if loser_id not in self.stats["users"]:
                self.stats["users"][loser_id] = {
                    "total": 0,
                    "wins": 0,
                    "losses": 0,
                    "win_streak": 0,
            ...
            user_stats = self.stats["users"][loser_id]
            user_stats["total"] += 1
            user_stats["losses"] += 1
            ...
            for winner_id in winner_ids:
                if winner_id not in self.stats["users"]:
                    self.stats["users"][winner_id] = {
                        "total": 0,
                        "wins": 0,
                        "losses": 0,
                        "win_streak": 0,
                ...
                winner_stats = self.stats["users"][winner_id]
            ...
            if is_pvp and len(winner_ids) == 1:
                ...
                if pvp_key not in self.stats["pvp"]:
                    self.stats["pvp"][pvp_key] = {
                        "total": 0,
                        f"{user1_id}_wins": 0,
                        f"{user2_id}_wins": 0
                    }
                
                pvp_stats = self.stats["pvp"][pvp_key]
                pvp_stats["total"] += 1
    ...
    def get_pvp_stats(self, user1_id: str, user2_id: str) -> Optional[Dict]:
        """获取两个用户之间的对战记录"""
        with self._lock:
    ...
    def get_top_players(self, group_id: str = None, min_games: int = 5, limit: int = 5) -> List[Tuple[str, float, Dict]]:
        """
        获取胜率排行榜
        :param group_id: 群组ID（暂时不使用，保留接口）
